...
function AddCGPACols(&$conn)
{
    ...
    $conn->query($sql1);
      $sql3 = "ALTER TABLE stud_data ADD PERCENTAGE FLOAT";
    $conn->query($sql3);
...
{   
    ...
    function getPercentage(&$conn)
{
    $sql = "SELECT STUDENT_ID,PHYSICS,CHEMISTRY,MATHEMATICS,ENGLISH,COMPUTER FROM `stud_data`";
    $result = $conn->query($sql);
    ...
    if ($result->num_rows>0) 
    {
        ...
        while($row=$result->fetch_assoc()) 
        {   
            $PhyMarks = $row["PHYSICS"];
            $ChemMarks = $row["CHEMISTRY"];
            $MathMarks = $row["MATHEMATICS"];
            $EngMarks = $row["ENGLISH"];
            $CompMarks = $row["COMPUTER"];
            
            $per = ($PhyMarks + $ChemMarks + $MathMarks + $EngMarks + $CompMarks)/5;
            $sql2 = "UPDATE stud_data SET PERCENTAGE=".$per." WHERE STUDENT_ID='".$row["STUDENT_ID"]."'";
            $result2 = $conn->query($sql2);
